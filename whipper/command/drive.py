...
class Analyze(BaseCommand):
    ...
    def do(self):
        ...
        if t.defeatsCache is None:
            print('Cannot analyze the drive: is there a CD in it?')
            return
        if not t.defeatsCache:
            print('cdparanoia cannot defeat the audio cache on this drive.')
        else:
            print('cdparanoia can defeat the audio cache on this drive.')

        info = drive.getDeviceInfo(self.options.device)
        if not info:
            print('Drive caching behaviour not saved: '
                  'could not get device info (requires pycdio).')
            return

        print('Adding drive cache behaviour to configuration file.')
    ...
    def do(self):
        ...
        for path in paths:
            ...
            try:
                ...
                print("       Unknown whether audio cache can be defeated. "
